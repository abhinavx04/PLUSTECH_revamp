...
float lineFn(vec2 st, float width, float perc, float offset, vec2 mouse, float time, float amplitude, float distance) {
    ...
    // Start amplitude influence earlier to avoid a flat left edge
    float amplitude_normal = smoothstep(max(0.0, split_point - 0.25), 0.6, st.x);
    ...
    // Ensure some base noise on the left by biasing the mix factor slightly up
    float noiseMix = clamp(st.x * 0.25 + 0.12, 0.0, 1.0);
    float xnoise = mix(
...
const Threads: React.FC<ThreadsProps> = ({
  ...
  return <div ref={containerRef} className="w-full h-full relative overflow-visible" {...rest} />;
};
