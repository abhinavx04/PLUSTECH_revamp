...
const AnnualReturnManager: React.FC = () => {
  ...
  const stableSetFormData = useCallback((d: typeof formData) => setFormData(d), []);
  const { clearDraft } = useFormDraft({
    key: 'annual-returns',
    editingId: editingReturn,
    formData,
  ...
  const filteredReturns = useMemo(() => {
    ...
    return annualReturns.filter((r) => r.financialYear.toLowerCase().includes(q) || r.formType.toLowerCase().includes(q));
  ...
  return (
    <div className="space-y-6">
      ...
      <SlidePanel
        ...
        title={editingReturn ? 'Edit Annual Return' : 'Create New Annual Return'}
        footer={
          <div className="flex flex-wrap gap-3">
            <button type="submit" form="annual-return-form" disabled={uploadingPDF || extractingData} className="px-6 py-2 bg-[#00aeef] text-black rounded-lg hover:bg-[#0099d4] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      >
        ...
        <form id="annual-return-form" onSubmit={handleSubmit} className="space-y-4">
          {/* PDF Upload Section */}
          <div>
      ...
      {/* Search + Inline Loading */}
      <div className="flex flex-wrap items-center gap-3">
        <input
          type="text"
          value={searchQuery}
          onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }}
          placeholder="Search by financial year or form type…"
          className="flex-1 min-w-[200px] px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#00aeef]"
        />
        {loading && (
          <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#00aeef] shrink-0"></div>
        )}
      </div>
      ...
      <div className="space-y-1">
        {paginatedReturns.length === 0 ? (
          ...
          paginatedReturns.map((returnData) => (
            <div key={returnData.id} className="flex items-center justify-between gap-3 px-4 py-3 rounded-lg hover:bg-white/5 transition-colors border border-transparent hover:border-white/10">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <span className={`w-2 h-2 rounded-full shrink-0 ${returnData.status === 'published' ? 'bg-green-400' : 'bg-yellow-400'}`} />
                <span className="text-white font-medium truncate">FY {returnData.financialYear}</span>
                <span className="px-2 py-0.5 text-xs rounded bg-white/10 text-gray-300 border border-white/10 shrink-0">{returnData.formType}</span>
                <span className="hidden md:inline text-xs text-gray-400 shrink-0">Turnover: ₹{returnData.turnover.toLocaleString()}</span>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => returnData.documentUrl && window.open(returnData.documentUrl, '_blank')} disabled={!returnData.documentUrl} className="px-2.5 py-1 text-xs text-gray-300 hover:text-white hover:bg-white/10 rounded transition-colors disabled:opacity-40">PDF</button>
                <button onClick={() => handleEdit(returnData.id!)} className="px-2.5 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">Edit</button>
                <button onClick={() => handleTogglePublish(returnData.id!, returnData.status)} className="px-2.5 py-1 text-xs bg-emerald-600 text-white rounded hover:bg-emerald-700 transition-colors">{returnData.status === 'published' ? 'Unpublish' : 'Publish'}</button>
                <button onClick={() => handleDelete(returnData.id!)} className="px-2.5 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700 transition-colors">Delete</button>
              </div>
            </div>
          ))
        )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-2">
          <button
            ...
            className="px-3 py-1 text-sm rounded bg-white/10 text-gray-300 hover:bg-white/20 disabled:opacity-40 transition-colors"
          >
            Prev
          </button>
